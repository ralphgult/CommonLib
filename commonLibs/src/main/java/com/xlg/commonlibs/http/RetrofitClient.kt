...
import com.xlg.commonlibs.ext.KEY_TOKEN
import com.xlg.commonlibs.ext.debugLogE
import com.xlg.commonlibs.ext.readData
import com.xlg.commonlibs.ext.serverPath
import com.xlg.commonlibs.utils.NetConfig
import com.xlg.commonlibs.utils.SSLSocketFactoryUtils.TrustAllHostnameVerifier
import com.xlg.commonlibs.utils.SSLSocketFactoryUtils.createSSLSocketFactory
import com.xlg.commonlibs.utils.SSLSocketFactoryUtils.createTrustAllManager
...
class RetrofitClient(mContext: Context, baseUrl: String) {
    ...
    init {
        ...
        val gson = GsonBuilder().registerTypeAdapterFactory(NullStringToEmptyAdapterFactory())
            .create()
        val finalUrl =
            if (baseUrl.startsWith("http")) baseUrl else serverPath + baseUrl
    ...
    inner class LoggingInterceptor : Interceptor {
        ...
        override fun intercept(chain: Interceptor.Chain): Response {
            ...
            "==========================================请求打印===========================================".debugLogE()
            if ("POST" == method) {
                "=请求地址：${request.url}".debugLogE()
            }
            "=headers: ${request.headers}".debugLogE()
            ("=请求结果:  $content").debugLogE()
            "==================================请求耗时:  $duration 毫秒===================================".debugLogE()
            return response.newBuilder().body(content.toResponseBody(mediaType)).build()
        }
    }
